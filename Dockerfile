...
RUN DEBIAN_FRONTEND=noninteractive apt-get -y update && \
    apt-get -y install git wget nginx-full php8.2-fpm fcgiwrap apache2-utils && \
    apt-get clean
...
RUN sed -i 's/user = www-data/user = git/g' /etc/php/8.2/fpm/pool.d/www.conf && \
    sed -i 's/group = www-data/group = git/g' /etc/php/8.2/fpm/pool.d/www.conf && \
    sed -i 's/listen.owner = www-data/listen.owner = git/g' /etc/php/8.2/fpm/pool.d/www.conf && \
    sed -i 's/listen.group = www-data/listen.group = git/g' /etc/php/8.2/fpm/pool.d/www.conf
